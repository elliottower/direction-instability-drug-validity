...
propose a construct-validity framework (a ``validity ladder'') with
seven conditions, of which three are tested empirically, that
specifies which interpretive threats a score has survived.
...
instability.}
Pan-HDAC inhibitors \citep{bolden2006anticancer} (panobinostat,
vorinostat, belinostat) achieve the lowest direction instability
($D = 0.14$--$0.19$); Class~I-selective entinostat is intermediate
($D = 0.27$); isoform-selective HDAC6 and HDAC8 inhibitors (tubacin,
PCI-34051 \citep{balasubramanian2008hdac8}) have the highest instability
within the class ($D = 0.32$--$0.45$). This gradient, reproduced from
...
warranted in concluding from it. The rung ordering reflects logical
dependency: a reliable measurement (Rung~2) is prerequisite for meaningful
projection (Rung~5), and localized signal (Rung~4) precedes phenotype
alignment (Rung~5). This ordering captures logical prerequisites rather
...
immaterial for the confirmed results: H3 ($p < 10^{-27}$) and H5
($p = 8.2 \times 10^{-13}$) survive Holm--Bonferroni correction even
under a single 8-hypothesis family. Experiment~8
...
0.83 was calibrated to the HDAC selectivity gradient (Figure~\ref{fig:hdac}):
the finest within-class rank ordering that direction instability
resolves. The ranking-disruptive threshold of 0.70 was set as a
...
association. The near-zero \emph{raw}-bracket correlation
($\rho = -0.04$) argues against this: if platform sharing drove the
result, raw bracket should also correlate with shRNA enrichment, since
...
but reveals confound-dominated core-machinery genes}
...
transcriptomic signatures. As noted in Section~4.1, the 78\% attrition
from the full JUMP-CP dataset (116{,}750 to 25{,}254 compounds) means
the retained set is biased toward well-replicated compounds; the result
may not generalize to sparsely-profiled compounds.
...
features), yet population $\rho = 0.987$.}
...
endpoint. The consequential validity facet (whether warrant upgrades

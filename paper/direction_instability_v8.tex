...
training consensus. Transport-stable bracket reverses this relationship in
the three rare-cell-line folds (HCC515, HEPG2, MCF7), where TS becomes
positively predictive, and attenuates the anti-prediction in the two common
folds (A375, A549), where both metrics remain negative but TS improves over
raw. The advantage is largest where the confound is strongest: in rare cell
lines where a single outlier can dominate the raw metric. This occurs
because high raw instability conflates genuine directional inconsistency
with Fr\'{e}chet variance from a small number of contexts; TS separates
these by penalizing the variance component.
...
drops from $+0.60$ to $+0.28$ after correction, confirming that the
...
tail of compounds with low cross-source consistency)---comparable to the
...
is consistent with the nature of the removed features: 743 health features
constitute 23\% of the 3{,}180-feature space---proportionally six times
larger than the drug toxicity gene set (41 of 978 genes, 4\%)---yet produce
less ranking impact. That a proportionally larger feature removal barely
moves the ranking is stronger evidence for the thesis than the drug case:
the health features carry less of the cross-source variance in
morphological signatures than stress genes carry in transcriptomic
signatures.
...
0.99$). The ranking is robust across all three domains, though the
Perturb-seq result involves a degenerate $K = 2$ pairwise comparison
(single Grassmannian geodesic distance) rather than the genuine $K \geq 5$
angular-consistency computation used in LINCS and JUMP-CP. The
qualitative conclusion---correction preserves ranking---holds across
domains, but the Perturb-seq domain tests a weaker geometric estimand.
...
both cell types. A label-permutation null ($p_{\text{perm}} < 10^{-4}$,
10{,}000 shuffles) confirms the essential/non-essential divergence exceeds
background K562/RPE1 divergence by ${\sim}6\times$ the null half-width,
ruling out the library-overlap confound directly rather than by argument
from attenuation. A genome-wide Perturb-seq screen with genuine
non-essential controls would provide an even sharper test.

The HP1 decision threshold ($\rho \geq 0.83$ = ``doesn't bite'') is
borrowed from the HDAC drug gradient in the companion paper
\citep{tower2026bracket}. This cutoff was pre-registered before
the Perturb-seq analysis, but it was calibrated on a different assay
and domain; a threshold derived from genetic-screen benchmarks would
be more principled.

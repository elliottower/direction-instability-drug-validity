...
Painting) and classifies drugs by MOA on a purely pharmacological label
(AUROC $= 0.821$ $[0.781, 0.858]$ vs.\ Fr\'{e}chet $0.722$, $\Delta =
+0.099$ $[+0.063, +0.135]$; post-hoc exploratory, robust across three
nuclear-receptor handling variants). A positive control validation
confirms that DI detects genuine context-dependence: DI is significantly
higher on nuclear-receptor drugs (known cell-type-specific) than on
constitutive-machinery drugs ($\Delta = +0.139$ $[+0.107, +0.173]$),
completing both halves of assay validation (specificity and sensitivity).
DI also reveals an HDAC inhibitor
selectivity gradient visible only through directional analysis. Three pre-registered experiments
...
\paragraph{AUPRC and class imbalance.}
The cosine outcome has a 6.0\% positive rate (162 of 2{,}694 drugs
produce consistent held-out signatures at the $>0.5$ threshold). At this
base rate, AUROC is inflated by easy negatives: both DI (0.969) and
Fr\'{e}chet (0.976) appear near-ceiling. AUPRC, which is sensitive to
precision at the positive end, gives a more honest comparison. On the
cosine outcome, DI and Fr\'{e}chet are statistically tied (AUPRC $0.814$
vs.\ $0.811$, paired $\Delta = +0.003$, $[-0.014, +0.019]$, $p = 0.37$).
On the construction-neutral Jaccard outcome (50.0\% positive rate, no
class imbalance), DI's advantage is clear: AUPRC $0.948$ $[0.940,
0.956]$ vs.\ $0.897$ $[0.885, 0.908]$ ($\Delta = +0.052$ $[+0.044,
+0.060]$, $p < 10^{-4}$). Fr\'{e}chet's marginal cosine win
(AUROC $\Delta = -0.007$) vanishes when measured by the metric that
...
\section{Post-hoc Exploratory: MOA Classification on a Non-geometric Outcome}
\label{sec:moa_auroc}

\textbf{Status: POST-HOC EXPLORATORY. Not pre-registered. Classification
rule frozen (commit \texttt{57867de}) before execution.}

The benchmark (Section~\ref{sec:benchmark}) uses Jaccard overlap and
cosine consistency as outcomes. A reviewer could object that DI's
advantage reflects shared magnitude-invariance with the Jaccard outcome,
since both are functions of set overlap rather than vector magnitude.
To address this, we evaluate DI as a predictor of a purely
pharmacological label: mechanism of action class.

\subsection{Classification rule}

We define a single-principle binary classification of LINCS drugs.
\emph{Machinery}: drugs targeting core genomic, epigenetic, proteostatic,
or cytoskeletal components (DNA repair, chromatin modification, cell
cycle, proteasome, tubulin). These targets are constitutively expressed
in dividing cells. \emph{Receptor}: drugs targeting cell-surface
receptors (GPCRs, receptor tyrosine kinases) or ion channels. These
targets are differentially expressed across cell types. All other
targets---intracellular enzymes outside core machinery (COX, PDE, MAO),
bacterial targets, transporters---are excluded automatically by the
rule. Nuclear hormone receptors are excluded as an edge case
(intracellular transcription factors with tissue-specific expression);
a sensitivity analysis tests both reassignments.

This rule yields $n = 180$ machinery and $n = 585$ receptor drugs from
the 8{,}949-drug LINCS dataset, with 153 kinase inhibitors held as a
separate intermediate category and 864 excluded. The classification
was frozen (commit \texttt{57867de}) before any results were observed,
with the keyword lists, priority ordering, and nuclear-receptor handling
specified in a machine-readable spec. This broader definition differs
from the narrow per-class examples in Section~5.3 ($n = 114$, $481$);
the narrow definition replicated the original drug paper's individual
MOA classes, while the broad definition applies a stated principle.

\subsection{Results}

Direction instability classifies drugs as machinery vs.\ receptor with
AUROC $= 0.821$ (95\% CI $[0.781, 0.858]$), AUPRC $= 0.667$
$[0.593, 0.737]$ (base rate 23.5\%). Fr\'{e}chet variance achieves
AUROC $= 0.722$ $[0.670, 0.772]$, AUPRC $= 0.569$ $[0.494, 0.646]$.
The paired difference is $\Delta_{\text{AUROC}} = +0.099$ $[+0.063,
+0.135]$, $p < 10^{-4}$ (10{,}000 paired bootstrap resamples,
percentile method).

Magnitude CV is significantly associated with MOA class in the
\emph{opposite} direction (AUROC $= 0.367$ $[0.324, 0.413]$; ordinal
$\rho = -0.174$, $p = 10^{-7}$): machinery drugs tend to have
\emph{higher} magnitude CV. This opposite-direction relationship rules
out the possibility that DI's advantage is attributable to shared
magnitude sensitivity, since the magnitude axis carries information
about these classes in the wrong direction to explain DI's win.

\subsection{Robustness: nuclear-receptor sensitivity}

The robustness gate requires significance in the primary analysis (CI
excludes zero) and directional consistency in both sensitivity variants.
All three hold:

\begin{table}[h]
\centering
\caption{MOA classification AUROC across nuclear-receptor handling
variants. All three show DI $>$ Fr\'{e}chet with CIs excluding zero.}
\label{tab:nhr_sensitivity}
\begin{tabular}{lrrccc}
\toprule
Variant & $n_{\text{mach}}$ & $n_{\text{recv}}$ & DI AUROC & Fr\'{e}chet & $\Delta$ [95\% CI] \\
\midrule
NHR excluded (primary) & 180 & 585 & 0.821 & 0.722 & $+0.099$ [$+0.063$, $+0.135$] \\
NHR $\to$ machinery & 305 & 585 & 0.719 & 0.615 & $+0.105$ [$+0.077$, $+0.132$] \\
NHR $\to$ receptor & 180 & 710 & 0.816 & 0.725 & $+0.090$ [$+0.055$, $+0.126$] \\
\bottomrule
\end{tabular}
\end{table}
...
Using all three categories (machinery $= 0 <$ kinase $= 1 <$ receptor
$= 2$; $n = 918$), DI shows the strongest monotonic relationship with
MOA class (Spearman $\rho = +0.492$ $[+0.438, +0.543]$, $p = 4 \times
10^{-57}$), compared to Fr\'{e}chet ($\rho = +0.320$) and magnitude CV
($\rho = -0.174$). The positive sign---higher DI for receptor-mediated
drugs---is consistent with the hypothesis that cell-surface targets,
being differentially expressed, produce more variable transcriptional
responses across cell types. This causal interpretation is plausible
but not directly tested.


\section{Sensitivity Validation: DI Detects Known Context-Dependence}
\label{sec:sensitivity}

A variability metric is only interpretable if it can both reject
spurious variation and detect genuine variation. The confound analysis
(Part~I) establishes the former. To establish the latter, we require a
positive control: perturbations known on independent grounds to be
context-dependent, on which DI must register elevated values. Absent
this, a null DI value is ambiguous---it could mean ``stable direction''
or ``instrument too blunt to see the instability.'' This is the standard
sensitivity and limit-of-detection requirement from assay validation,
independent of any external framework.

\subsection{Positive and negative controls}

Nuclear hormone receptor (NHR) agonists serve as the positive control.
NHR transcriptional output is famously cell-type-specific: the same
...
textbook endocrinology, established decades before any transcriptomic
assay. NHR drugs ($n = 125$) are therefore an externally-defined set
whose perturbation signatures should vary across cell lines. These drugs
were excluded from the MOA classification (Section~\ref{sec:moa_auroc});
here they serve a different experimental role as a sensitivity control.

The negative control comprises proteasome, tubulin, HDAC, and
topoisomerase inhibitors ($n = 59$)---four mechanistically distinct
target classes that all hit constitutive, universally expressed cellular
machinery. The hypothesis direction (DI higher in NHR than in
machinery) was committed from pharmacology before examining any DI
values (spec frozen at commit \texttt{49bb198}).

\subsection{Results}

DI is significantly higher in NHR drugs (mean $D = 0.937$) than in
constitutive machinery drugs (mean $D = 0.797$), with a mean
difference of $+0.139$ $[+0.107, +0.173]$ (10{,}000 bootstrap
resamples, percentile CI, $p < 10^{-6}$; pre-registered primary test).
As a secondary discrimination metric, AUROC $= 0.837$ $[0.755, 0.908]$;
this is a different contrast from the MOA classification
(Section~\ref{sec:moa_auroc}), which separates machinery from receptor
drugs across the full annotated set, and the two results should not be
conflated.

The result is robust. Restricting to drugs measured in $\geq 10$ cell
lines ($n_{\text{NHR}} = 103$, $n_{\text{mach}} = 51$) \emph{strengthened}
the effect ($+0.153$ $[+0.117, +0.190]$), the signature of a real signal
being cleaned up by better measurement rather than a small-$n$ artifact.
The narrower negative control (proteasome and tubulin only, $n = 22$)
also held ($+0.087$ $[+0.050, +0.128]$).

\subsection{Instrument characterization}

A synthetic spike-in provides quantitative instrument characterization.
Pseudo-perturbations are constructed in $\mathbb{R}^{978}$ with
controlled angular spread $\sigma$ (half-normal scale) and isotropic
measurement noise $\tau$ calibrated to the negative-control DI
distribution (see Appendix). DI is monotonically increasing in
$\sigma$---the basic sanity check that the metric responds to true
instability---and the limit of detection (smallest $\sigma$ at which
$\geq 95\%$ of pseudo-perturbations exceed the null threshold) improves
with more contexts: $\sigma_{\text{LOD}} = 0.65$ at $K = 5$ cell lines,
$0.35$ at $K = 13$, and $0.25$ at $K = 40$. Because the isotropic
noise model underrepresents real inter-context variance (std ratio $=
17.7$), these LOD values are a lower bound on the real-data detection
floor, not a direct estimate.

Combined with the confound analysis, this completes both halves of
assay validation: DI does not fire spuriously (specificity) and DI does
fire on genuine context-dependence (sensitivity). Every null DI value
in this paper is now interpretable as real stability, not instrument
blindness.
...
passed. A post-hoc exploratory analysis extends this to MOA
classification: DI classifies drugs as machinery vs.\ receptor with
AUROC $= 0.821$ $[0.781, 0.858]$, outperforming Fr\'{e}chet variance
($\Delta = +0.099$ $[+0.063, +0.135]$, $p < 10^{-4}$) on a label
defined by target biology rather than transcriptomic geometry
(Section~\ref{sec:moa_auroc}). Magnitude CV predicts MOA class in the
opposite direction ($\rho = -0.174$), ruling out the interpretation
that DI's advantage is a repackaging of magnitude sensitivity. A
positive control validation (Section~\ref{sec:sensitivity}) closes the
complementary gap: DI is significantly elevated on nuclear-receptor
drugs---known from endocrinology to be cell-type-specific---compared to
constitutive-machinery drugs ($\Delta = +0.139$ $[+0.107, +0.173]$),
confirming that DI detects genuine context-dependence and that null
values reflect real stability rather than instrument blindness. The criteria that failed are interpretable: the Perturb-seq
...
classifies drugs by mechanism of action on a non-geometric label
(AUROC $= 0.821$ $[0.781, 0.858]$), detects known context-dependence
in nuclear-receptor drugs ($\Delta = +0.139$ $[+0.107, +0.173]$ vs.\
constitutive machinery), replicates MOA stratification across
measurement technologies, and resolves mechanistic gradients within drug
classes. Both halves of assay validation are now closed: DI does not
fire spuriously (specificity) and fires on genuine context-dependence
(sensitivity). The metric works because it asks the right question---do
perturbation effects point in the same direction?---rather than a
question contaminated by how precisely we estimated them.
...
\begin{table}[h]
\centering
\caption{MOA class sizes used in stratification analyses
(Part~II MOA stratification and Experiment~3 JUMP replication). LINCS
classification uses Drug Repurposing Hub annotations with keyword
matching; JUMP-CP uses the same MOA lists applied to
InChIKey-matched compounds.}
\label{tab:moa-narrow}
\begin{tabular}{lrr}
\toprule
...
\begin{table}[h]
\centering
\caption{MOA class sizes for the binary classification analysis
(Section~\ref{sec:moa_auroc}). The broad principled
definition classifies core genomic, epigenetic, proteostatic, and
cytoskeletal targets as \emph{machinery} and cell-surface receptors and
ion channels as \emph{receptor}. All 125 nuclear-receptor drugs are
deterministically excluded (NHR check runs first). Kinase inhibitors
form an intermediate class used only in the ordinal analysis.
Spec frozen at commit \texttt{57867de}.}
\label{tab:moa-broad}
\begin{tabular}{lrr}
\toprule
MOA class & $n$ & $\%$ of annotated \\
\midrule
Machinery (HDAC, proteasome, mTOR, topoisomerase, tubulin, \ldots) & 180 & 10.1 \\
Kinase inhibitors (ordinal only) & 153 & 8.6 \\
Receptor (GPCR, ion channel, cytokine receptor, \ldots) & 585 & 32.8 \\
Nuclear receptor (excluded) & 125 & 7.0 \\
Other excluded & 739 & 41.5 \\
\bottomrule
\end{tabular}
\end{table}

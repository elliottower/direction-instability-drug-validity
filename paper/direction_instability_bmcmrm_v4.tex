...
\subsection{What direction instability identifies}

Under the DAG of Figure~\ref{fig:dag}, cosine normalization is an
adjustment that blocks the magnitude path from perturbation strength to
the metric. When that adjustment is exact, $D$ identifies the
direction-only dependence of the biological signal on context---the direct
path, marginalizing over magnitude. The adjustment is not exact in one
regime: when a signature's norm approaches zero, unit-normalization
amplifies noise and the blocked path partially reopens, so $D$ no longer
cleanly identifies the direct effect. We exclude near-zero-norm
signatures ($\|\mathbf{s}\| < 10^{-10}$, guarded in code), but
low-magnitude perturbations remain the boundary condition where the DAG's
identification claim is weakest. We have not proven identification with
do-calculus; the DAG is a heuristic justification for cosine normalization
as a confound-blocking adjustment, not a formal identification theorem.

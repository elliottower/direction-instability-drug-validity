...
\citep{subramanian2017next,keenan2018library}. We retained 8{,}949
...
reflects genuine independence: the two domains use different organisms
(human cell lines vs.\ two specific lines), different perturbation types
...
The shRNA consensus averages across ${\sim}20$ cell lines, producing a
...
further attenuates the correlation. A convergent-validity
...
a philosophical abstraction. The asymmetry is deliberate: failing a
...
mechanisms. Cosine normalization removes the linear magnitude
dependence from direction instability (right panel) within the
fixed-dose-across-context regime. \textbf{Left:}~DepMap variance-based
...
\textbf{Right:}~Cosine normalization blocks the magnitude path, so the
observed association reflects biological signal.
The confound mechanisms differ across domains, yet the validity
...
changes from single-replicate noise in 978 dimensions.

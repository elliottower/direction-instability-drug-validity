...
gene. This metric normalizes by construction (Equation~1), eliminating
the mechanical path from magnitude to the metric.
...
\emph{four} metrics: geodesic distance ($d = -0.06$), subspace D ($d =
0.60 \rightarrow$ same as geodesic), PC1 cosine D ($d = -0.05$), and
PC1 residualized ($d = 0.07$, $p = 0.19$). The convergence of all four
on the same null---including the mechanically magnitude-invariant
cosine metric---confirms the entire signal traversed the
estimation-quality path.

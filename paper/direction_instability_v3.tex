...
correction preserves the ranking (LINCS HDAC selectivity gradient after
toxicity correction: $\rho = 0.83$; Perturb-seq global geodesic-distance
ranking: $\rho = 0.91$; JUMP-CP global morphological ranking: $\rho =
0.99$). The predicted failure modes are domain-specific: cytotoxic drugs are
already high-instability, while universal-essential gene knockdowns produce
\emph{divergent} responses across cell types (Cohen's $d = +0.55$, $p <
10^{-30}$). Despite global ranking stability, individual perturbations shift
thousands of rank positions after correction, identifying the specific
compounds or genes whose geometry is most confound-dominated. The validity
...
compound profiles across 10 imaging sources. We retained 25{,}254
compounds present in $\geq$5 sources, aggregated per compound $\times$
source by mean, and computed direction instability over the full 3{,}180
feature set. Cell-health features (743 features matching Area, Compactness,
...
after correction. The top 100 rank-shifters are enriched for spliceosomal
complex assembly (Fisher's exact OR $= 8.9$, $p = 1.3 \times 10^{-7}$) and
proteasomal catabolic processes (OR $= 4.1$, $p = 6.7 \times 10^{-5}$):
proteasome subunits (\emph{PSMA3}, \emph{PSMB6}, \emph{PSMC4}) shift
750--900 positions; spliceosome components (\emph{SF3B1}, \emph{SNRPG},
\emph{DDX23}) shift 660--820 positions. These core-machinery complexes are
the genes whose cross-cell-type geometry is most dominated by the shared
...
We tested two of five pre-registered drug-domain experiments (H1, H2),
three genetic-domain experiments (HP1, HP2, HP3), and one morphological
experiment (cell-health correction). Experiments H3--H5
...
consistent pattern across three domains, and because the honest nulls (H1,
...
three domains tested ($\rho = 0.83$ for the LINCS HDAC gradient, $0.91$
for Perturb-seq, and $0.99$ for JUMP-CP after correction; the global LINCS
ranking gives $\rho > 0.99$).
...
rules out shared lethal-response geometry in genetic knockdowns; cell-health
correction rules out viability-driven consistency in morphological profiles.

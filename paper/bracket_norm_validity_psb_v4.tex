...
(Wilcoxon signed-rank on paired fold-level differences,
$p = 8.2 \times 10^{-13}$; mean
...
the projection failing. The positive raw-bracket/CRISPRi correlation
likely arises because K562 is a leukemia line where many drug targets
are highly expressed; drugs with high raw instability across many cell
lines coincidentally match K562-specific CRISPRi signatures, producing
a spurious correlation that the phenotype projection amplifies rather
than removes. On the 114-drug matched subset, shRNA
...
across ${\sim}20$ cell lines. A post-hoc power calculation shows
$>$99\% power to detect $\rho = 0.376$ at $n = 131$ ($\alpha = 0.05$,
one-sided); the CRISPRi 95\% CI $[-0.29, 0.05]$ excludes the shRNA
effect size entirely, and the two correlations differ significantly
($z = 5.47$, $p < 10^{-7}$). The CRISPRi result is therefore a
genuine divergence from the shRNA finding, not an underpowered null.
The K562-only confound is the pre-registered explanation for this
divergence: drugs tested across many cell lines are poorly matched by
an on-target direction from a single hematopoietic context.
...
effects. Transport stability (H5) proves
...
$p = 0.16$, $n = 131$; 95\% CI excludes shRNA $\rho = 0.376$,
$z = 5.47$, $p < 10^{-7}$); this genuine divergence is attributable to
the K562-only confound (CRISPRi signatures from a single cell type are
poor on-target directions for drugs acting in non-hematopoietic contexts).
The shRNA H3 result is not refuted but neither is it reinforced. Drugs with unannotated targets

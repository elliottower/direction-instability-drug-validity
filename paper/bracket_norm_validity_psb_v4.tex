...
held-out cell-line effects in all 66 valid leave-one-out folds
($p_{\text{Wilcoxon}} = 8.2 \times 10^{-13}$; mean
$\Delta\rho = +1.10$, 95\% CI $[1.00, 1.19]$), with a
...
A CRISPRi convergent-validity check does not independently confirm the
phenotype-projection result ($\rho = -0.12$, $p = 0.16$, $n = 131$),
attributable to a pre-registered single-cell-line confound: CRISPRi
enrichment correlates with raw bracket rather than projected bracket,
the reverse of the shRNA pattern.
...
\textbf{Pre-registered prediction (H5):} TS outpredicts raw bracket
in $\geq 80\%$ of valid folds (co-primary~1); Wilcoxon signed-rank on
per-fold $\Delta\rho$ rejects $H_0$: median $\leq 0$ at $\alpha = 0.05$
(co-primary~2).
...
H5 & TS wins $\geq$80\% folds & 66/66 folds, $p_W = 8.2\mathrm{e}{-13}$ & Confirmed & Yes \\
\midrule
...
\paragraph{CRISPRi convergent-validity check.}
...
$n = 131$), corresponding to pre-registered outcome~(c). The
pre-registered secondary criterion identifies the likely cause: raw
bracket correlated positively with CRISPRi enrichment
($\rho = +0.27$, $p = 0.002$) while projected bracket did not---the
reverse of the shRNA pattern---indicating that the K562-only CRISPRi
directions are confounded by cell-type-specific effects rather than
the projection failing. On the 114-drug matched subset, shRNA
projection ($\rho = 0.42$) and CRISPRi projection ($\rho = -0.14$)
diverge, confirming that the ground-truth source drives the difference.

We report CRISPRi as not providing independent convergent validity for
H3, attributable to the pre-registered single-cell-line limitation.
The shRNA-based H3 result stands on its original evidence rather than
being reinforced. CRISPRi signatures derive from a single hematopoietic
cell type, so the CRISPRi ``on-target direction'' captures
cell-type-specific effects rather than cross-context mechanism---a
confound that multi-cell-line shRNA consensus avoids by averaging
across ${\sim}20$ cell lines. The non-significant CRISPRi projected
$\rho$ is underpowered ($n = 131$ vs.\ 795 for shRNA) and should not
be interpreted as evidence against the projection method.
...
\paragraph{Full leave-one-out summary.}
Across 66 folds, mean raw $\rho = -0.60$ (anti-predictive in every
fold) while mean TS $\rho = +0.50$ (positively predictive). Mean
$\Delta\rho = +1.10$ (95\% bootstrap CI $[1.00, 1.19]$). Five cell
lines were excluded for having $< 20$ eligible drugs. Table~\ref{tab:transport}
shows representative folds illustrating the rare/common regime distinction.
...
A variance-preserving permutation null (on the preliminary 5-fold analysis)
tests whether the TS advantage is algebraic: held-out
...
check does not independently confirm this, attributable to the K562-only confound. Transport stability (H5) proves
...
a conservative estimate. A CRISPRi convergent-validity check did not
independently confirm the phenotype-projection result ($\rho = -0.12$,
$p = 0.16$, $n = 131$); this is attributable to the K562-only confound
(CRISPRi signatures from a single cell type are poor on-target directions
for drugs acting in non-hematopoietic contexts), but the shRNA H3 result
is un-refuted rather than reinforced. Drugs with unannotated targets
...
($\rho = 0.38$ vs.\ $-0.04$); transport-stable bracket outpredicts raw
bracket in all 66 leave-one-out folds ($p_{\text{Wilcoxon}} = 8.2 \times
10^{-13}$, mean $\Delta\rho = +1.10$); localization helps for intracellular

...
consistency projects onto its annotated target. Universal-essential knockdowns
diverge across cell types rather than converging on a shared lethal response
($d = 0.60$, $p_{\text{perm}} < 10^{-4}$), the opposite of the naive confound
expectation. Toxicity correction shifts scores by less
than $0.01$, refuting the predicted confound: cytotoxic compounds already
occupy the high-instability regime.

\textbf{Conclusions:}
Corrections leave the population ranking largely intact; they identify
which perturbations move under correction. A score adequate
...
review;][]{tower2026atlas}. Those two studies establish the score and its
behavior at scale. Whether its rankings survive correction for the confounds
that motivate them is the question here.
...
cell types, as \citet{tower2026atlas} reports for JUMP-CP plate replicates
within a single cell line. The JUMP-CP result is reported on that basis
throughout. A frozen block of
...
on the remaining 2{,}928 features.
...
We treat the five LINCS hypotheses (H1--H5) as one test family and the two
Perturb-seq hypotheses carrying pass/fail thresholds (HP1--HP2) as a second. The separation
...
Variance-regularized direction instability (TS) adjusts the raw score by
...
$\bar{\mathbf{s}}$ is the Fr\'echet mean direction. The alternative (a ratio) would be
...
values is reported in Section~\ref{sec:limitations}.
...
informative null.
In the Perturb-seq domain, HP3 gives directional support; HP1 shows the correction
...
H5 & TS wins $\geq$80\% folds & 66/66 on signed $\rho$; 10/66 on $|\rho|$ & Met (artifact) & --- & \textbf{Not confirmed} \\
\midrule
...
negatively, and the variance-regularized variant subtracts a term that
...
\citet{tower2026drug} reports the same prediction on this dataset under a
different estimator and fold structure (AUROC $= 0.986$ against a permutation
null of $0.500 \pm 0.012$); the leave-one-cell-line-out result here recovers it
...
different transcriptomic programs in K562 and RPE1. This is consistent with cell-line-specific
...
\citet{tower2026atlas} finds that mechanism-of-action class explains 20\% of
residualized instability variance across 55{,}000 perturbations
($\eta^2 = 0.201$), separating core-machinery inhibitors from
receptor-signaling drugs whose activity depends on target expression. A score
varying with how widely a target is expressed, rather than with a removable
nuisance axis, is one that feature and gene-set deletions should leave ordered.
That is what the three domains show.
...
variance---the basis for the variance-regularized correction
...
The three analyses compare the sensitivity of geometric rankings to
domain-specific adjustments, but they use different geometric estimands and
context axes (Table~\ref{tab:cross_domain}). In LINCS, $D$ measures directional
consistency across cell lines. Perturb-seq compares per-cell-type response
subspaces using Grassmannian geodesic distance. In JUMP-CP, $D$ measures
technical reproducibility across imaging sources. What the three
...
Cytotoxic drugs occupy the high-instability regime, and universal-essential
knockdowns produce divergent rather than shared response subspaces. Both
results oppose the naive expectation that destructive perturbations generate a
shared, apparently transportable response. The JUMP-CP analysis instead
measures the sensitivity of a cross-site technical-reproducibility ranking to
morphological feature deletion.
...
The variance-regularized formula (Eq.~\ref{eq:ts}) is a heuristic
...
LINCS measures 978 landmark genes, which restricts the toxicity gene set to
that panel.
...
TS: variance-regularized direction instability.

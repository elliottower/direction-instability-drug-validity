...
\title{A Pre-Registered Validation Framework for Geometric Perturbation Scores\\Across Transcriptomic, Genetic, and Morphological Profiling Data}
...
biological contexts---rank drugs and genes by mechanism conservation
across large-scale profiling datasets. These scores are biologically
informative, but the raw number conflates genuine mechanism conservation
with estimation artifacts: a drug scored from few cell lines may appear
consistent by chance, and cross-cell-type consistency may reflect
off-target programs rather than on-target mechanism. No systematic
framework exists for diagnosing which confounds a given score has survived.

\textbf{Results:}
We introduce a validation framework with seven conditions and test three
empirically across 8{,}949 LINCS L1000 drug compounds, 1{,}676
Perturb-seq CRISPRi gene knockdowns, and 25{,}254 JUMP-CP Cell Painting
morphological profiles using pre-registered hypotheses with
Holm--Bonferroni correction. A variance-regularized estimator
(transport-stable direction instability) outpredicted raw scores for
held-out cell-line effects in all 66 valid leave-one-out folds
(Wilcoxon $p = 8.2 \times 10^{-13}$; mean $\Delta\rho = +1.10$, 95\%
CI $[1.00, 1.19]$), with raw scores anti-predictive in every fold.
Phenotype-projected scores correlated with on-target genetic perturbation
connectivity ($\rho = 0.38$, $p < 10^{-27}$) while raw scores did not
($\rho = -0.04$). Essential-gene correction on Grassmannian subspaces
revealed that universal-essential knockdowns \emph{diverge} rather than
converge across cell types ($d = 0.60$, $p_{\text{perm}} < 10^{-4}$),
inverting the predicted confound and identifying proteasome/spliceosome
genes as confound-dominated. Domain-specific corrections preserved global
rankings in all three domains ($\rho > 0.91$) while reorganizing
individual items by up to 7{,}155 rank positions.

\textbf{Conclusions:}
The validation framework---define confounds, specify corrections,
pre-register predictions, test whether corrections reorganize rankings
or preserve them---applies to any geometric score measuring
perturbation-response consistency across contexts. Corrections preserve
population rankings while identifying individual perturbations most
susceptible to confounding, providing a structured, pre-registerable
diagnostic procedure for geometric perturbation scores.
\end{abstract}

\paragraph{Keywords:} direction instability, perturbation scoring, validation framework, LINCS L1000, Perturb-seq, Cell Painting, variance correction, pre-registration, Fr\'{e}chet variance, Grassmannian geodesic
...
perturbation-response consistency across contexts. We report honest nulls
alongside confirmations, because the pattern of where validation conditions
matter---and where they do not---is itself the finding.
...
\subsection{Experiment 5: Variance-regularized direction instability outpredicts raw scores in all 66 held-out cell lines}
\label{sec:exp5}
...
\subsection{Validation conditions preserve rankings while diagnosing confounds}
...
The three headline results each address a different confound.
Variance-regularized direction instability (H5) outpredicts raw scores in
all 66 leave-one-out folds ($p = 8.2 \times 10^{-13}$), with raw scores
anti-predictive in every fold---the strongest single result.
...
Variance-regularized direction instability outpredicts raw scores in all 66
leave-one-out folds ($p = 8.2 \times 10^{-13}$, mean
$\Delta\rho = +1.10$), with raw scores anti-predictive in every fold.
...
knockdowns, 25{,}254 JUMP-CP compounds---with domain-specific failure
